...
def IDFT(X):
    ...
    """
    # Calculate IDFT for real sinusoid

    kv = np.arange(len(X))  # same range as x
    x = np.zeros(len(X), dtype=complex)  # initialize the array with zeros (faster than allocating memory dynamically)
    for n in kv:
        s = np.exp(1j * 2 * np.pi * n / len(X) * kv)
        x[n] = 1.0 / len(X) * sum(X * s)
