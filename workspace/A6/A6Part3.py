...
"""
def estimateInharmonicity(inputFile = '../../sounds/piano.wav', t1=0.2, t2=0.4, window='hamming',
                            M=2047, N=2048, H=128, f0et=5.0, t=-90, minf0=130, maxf0=180, nH = 25):
    """
    ...
    w = get_window(window, M, M % 2 == 0)  # obtaining analysis window
    # 1. Use harmonic model to compute the harmonic frequencies and magnitudes
    hfreq, hmag, hphase = HM.harmonicModelAnal(x, fs, w, N, H, t, nH, minf0, maxf0, f0et, 0.01, 0.0)
    numFrames = len(hfreq[:, 0])
    n_partials = len(hfreq[0, :])
    frmTime = H * np.arange(numFrames) / float(fs)


    # 2. Extract the time segment in which you need to compute the inharmonicity.
    l1 = round(numFrames * t1/max(frmTime))
    l2 = round(numFrames * t2/max(frmTime))
    # 3. Compute the mean inharmonicity of the segment
    I_l = np.zeros((numFrames,))
    for l in range(numFrames):
        I = []
        R = [r for r in range(n_partials) if hfreq[l,r] > 0]
        for r in R:
            I.append(abs(hfreq[l, r] - (r + 1) * hfreq[l, 0]) / (r+ 1))
        I_l[l] = 1/len(R)*sum(I) if len(R)>0 else 0
    Imean = 1/(l2-l1+1) * sum(I_l[l1+1:l2+2])
    return Imean
print(estimateInharmonicity())