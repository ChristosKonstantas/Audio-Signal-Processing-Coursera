...
def segmentStableNotesRegions(inputFile = '../../sounds/cello-phrase.wav', stdThsld=10, minNoteDur=0.1,
                              winStable = 3, window='hamming', M=1025, N=2048, H=256, f0et=5.0, t=-100,
                              minf0=310, maxf0=650):
    """
    ...
    """
    fs, x = UF.wavread(inputFile)  # reading input file
    ...
    f0_cents = 1200 * np.log2(f0 / 55.0 + eps)

    # 2. Create an array containing standard deviation of last winStable samples
    std_devs = np.array([np.std(f0_cents[max(0, k - winStable + 1):k + 1]) for k in range(len(f0_cents))])

    # 3. Apply threshold on standard deviation values to find indexes of the stable points in melody
    stable_frames = np.where(std_devs < stdThsld)[0]

    # 4. create segments of continuous stable points such that consecutive stable points belong to same segment
    start = stable_frames[0]
    segments = []
    for j in stable_frames:
        if j+1 < len(stable_frames):
            if stable_frames[j] + 1 != stable_frames[j+1]:
                end = stable_frames[j]
                segments.append([start, end])
                start = stable_frames[j+1]
    # 5. apply segment filtering, i.e. remove segments with are < minNoteDur in length
    x_dur = len(x)/fs
    min_dur = minNoteDur*len(x)/x_dur
    min_dur_f0 = int(min_dur * len(f0_cents)/len(x))
    segments_filtered = np.array([s for s in segments if s[1]-s[0] >= min_dur_f0])

    plotSpectogramF0Segments(x, fs, w, N, H, f0, segments_filtered)  # Plot spectrogram and F0 if needed
