...
def suppressFreqDFTmodel(x, fs, N):
    ...
    """
    M = len(x)
    w = get_window('hamming', Nx=M, fftbins=M % 2 == 0)
    outputScaleFactor = sum(w)
    mX, pX = dftAnal(x, w, N)
    y = dftSynth(mX, pX, w.size) * outputScaleFactor
    b70 = np.ceil(70.0 * N / fs)  # index up to which the magnitude spectrum should be suppressed
    mX[:int(b70) + 1] = -120  # +1 because we want b70 to be included
