...
def optimalZeropad(x, fs, f):
    ...
    # number of samples per period of the sinusoid (samples / sec  /  cycles / sec  = samples / cycle)
    T = fs / f
    # minimum DFT length N that is a multiple of the period is the smallest multiple of T that is at least M
    N_min = int(np.ceil(M / T) * T)

    # next power of 2 greater than or equal to N_min
    # N = 2 ** int(np.ceil(np.log2(N_min)))

    # zero padding
    hM1 = int(np.floor((M + 1) // 2))
    hM2 = int(np.floor(M // 2))
    x_zero_padded = np.zeros(N_min)
    x_zero_padded[:hM1] = x[hM2:]
    x_zero_padded[-hM2:] = x[:hM2]
    # DFT
    X = fft(x_zero_padded)
    mX = 20 * np.log10(abs(X[:N_min // 2 + 1]) + sys.float_info.min)  # Adding a small constant to avoid log(0)
