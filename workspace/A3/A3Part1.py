...
def minimizeEnergySpreadDFT(x, fs, f1, f2):
    ...
    # Calculate the periods of the sinusoids (samples/sec / cycles/sec = samples/cycle)
    T1_samples = int(fs / f1)
    T2_samples = int(fs / f2)

    # M = Least Common Multiple (LCM) of the sinusoid periods (in samples)
    # Period of sum of sinusoids
    M = int((T1_samples * T2_samples) / math.gcd(T1_samples, T2_samples))

    # take DFT for one period
    dft_x = fft(x[:M])

    mX = 20 * np.log10(abs(dft_x[:M // 2 + 1]))
    return mX
