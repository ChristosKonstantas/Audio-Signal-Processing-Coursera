...
def zpFFTsizeExpt(x, fs):
    ...
    N = 2*M
    xseg = x[:M]
    w1 = get_window('hamming', M, M%2==0)
    w2 = get_window('hamming', 2*M, True)
    # Case 1: Input signal xseg (256 samples) window w1 (256 samples) and FFT size of 256
    mX1, pX1 = dftAnal(xseg, w1, N=M)
    # Case 2: Input signal x (512 samples) window w2 (512 samples) and FFT size of 512
    mX2, pX2 = dftAnal(x, w2, N=N)
    # Case 3: Input signal x (256 samples) window w1 (256 samples) and FFT size of 512 -- zero-padding --
    mX3, pX3 = dftAnal(xseg, w1, N=N)
    plt.subplot(2, 1, 1)
    ...
    plt.subplot(2, 1, 2)
    plt.plot(mX1[0:80], color="blue", label="case-1")
    plt.plot(mX2[0:80], color="green", label="case-2")
    plt.plot(mX3[0:80], color="red", label="case-3")
    ...
    plt.tight_layout()
    plt.savefig('FFT_Spectrum_5.png')
    plt.show()

    return mX1[0:80], mX2[0:80], mX3[0:80]