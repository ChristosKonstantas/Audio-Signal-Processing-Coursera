...
def testRealEven(x):
    ...
    # zero phase windowed signal construction
    ...
    # check if the right half is equal to the left half to see if the signal is even (0 value would be the same)
    isEven = bool(np.all(np.real(X[1:hM1]) == np.real(X[N - 1:hM2:-1])))

    # check if the signal is real (i.e., imaginary part is close to zero)
    isReal = bool(np.all(np.abs(np.imag(X)) < 1.0e-6))

    # determine if the signal is real and even
    isRealEven = bool(isEven and isReal)
